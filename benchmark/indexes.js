...
if (!Number.isSafeInteger(rowCount) || rowCount <= 0 || !Number.isSafeInteger(updateCount) || updateCount <= 0) {
...
function run(label, indexColumns) {
  ...
  db.query('CREATE TABLE benchmark_rows (id int primary key, bucket number, payload string)')
  const values = []
