...
const { AofLog, PubSubHub, YasdClient, YasdServer, YASD, saveSnapshot } = mod;
...
async function runWorkload(label, count, workers, operation, details = {}) {
  ...
  await Promise.all(
    Array.from({ length: Math.min(workers, count) }, (_, workerIndex) =>
      worker(workerIndex),
    ),
  );
...
function seedRows(db, table, rowCount) {
  db.query(
    `CREATE TABLE ${table} (id int primary key, bucket number, payload string)`,
  );
  const batchSize = 500;
...
async function benchmarkUpdates() {
  ...
  try {
    return await runWorkload(
      ...
      (i) => {
        db.query(
          `UPDATE update_rows SET bucket = ${i % 100} WHERE id = ${i % rows}`,
        );
      },
    );
  } finally {
...
async function benchmarkPubSub() {
  ...
  for (let i = 0; i < subscriberCount; i++)
    hub.subscribe("fanout", () => undefined);
  try {
    return await runWorkload(
      "pubsub-fanout",
      operations,
...
async function benchmarkPersistence() {
  ...
  for (let i = 0; i < Math.min(rows, 1000); i++)
    db.set(`snapshot:${i}`, { value: i });
  const log = new AofLog(aofPath);
  try {
    const aof = await runWorkload(
      "persistence-aof-append",
      operations,
      concurrency,
      (i) => {
