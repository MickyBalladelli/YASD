...
function decodeFile(bytes: Buffer): string {
  ...
  if (!Buffer.from(raw, "utf8").equals(bytes))
    throw new DatabaseError("invalid persistence UTF-8", "PERSISTENCE_ERROR");
  return raw;
}
function readFileBounded(filePath: string): string {
  if (fs.statSync(filePath).size > MAX_PERSISTENCE_FILE_BYTES)
    throw new DatabaseError(
      "persistence file exceeds 256 MiB",
      "LIMIT_EXCEEDED",
    );
  return decodeFile(fs.readFileSync(filePath));
}
async function readFileBoundedAsync(filePath: string): Promise<string> {
  if ((await fs.promises.stat(filePath)).size > MAX_PERSISTENCE_FILE_BYTES)
    throw new DatabaseError(
      "persistence file exceeds 256 MiB",
      "LIMIT_EXCEEDED",
    );
...
const snapshotQueues = new Map<
  string,
  { tail: Promise<unknown>; depth: number }
>();
function queueSnapshot<T>(
  filePath: string,
  operation: () => Promise<T>,
): Promise<T> {
  ...
  if (!queue) {
    ...
    snapshotQueues.set(key, queue);
  }
  if (queue.depth >= 64)
    return Promise.reject(
      new DatabaseError("snapshot queue limit exceeded", "LIMIT_EXCEEDED"),
    );
  queue.depth++;
  const current = queue;
      ...
      return (
        ...
        value.entries.every(
          (entry) =>
            ...
            typeof entry.key === "string" &&
            hasOwn(entry, "value") &&
            (!hasOwn(entry, "expiresAt") || isFiniteNumber(entry.expiresAt)),
        )
      );
    case "persist":
...
export async function saveSnapshot(
  ...
  const aofSeq =
    options.aofSeq === undefined
      ? undefined
      : checkAofSeq(options.aofSeq, "aofSeq");
  ...
  if (Buffer.byteLength(encoded) > MAX_PERSISTENCE_FILE_BYTES)
    throw new DatabaseError("snapshot exceeds 256 MiB", "LIMIT_EXCEEDED");
...
export function loadSnapshot(
  ...
  return queueSnapshot(filePath, () =>
    loadSnapshotUnlocked(store, filePath, options),
  );
}
...
async function loadSnapshotUnlocked(
  ...
  if (
    ...
    (file.aofSeq !== undefined &&
      (!Number.isSafeInteger(file.aofSeq) || file.aofSeq < 0))
  ) {
    throw new Error(`invalid snapshot file: ${filePath}`);
  }
  const count = clearFirst
    ? store.replace(file.entries)
    : store.restore(file.entries);
  ...
  switch (op.op) {
    case "set":
      if (Object.prototype.hasOwnProperty.call(op, "expiresAt")) {
        cache.setAt(
          op.key,
          op.value,
          op.expiresAt === null ? undefined : op.expiresAt,
        );
      } else {
      ...
      for (const entry of op.entries) {
        if (Object.prototype.hasOwnProperty.call(entry, "expiresAt")) {
          cache.setAt(
            entry.key,
            entry.value,
            entry.expiresAt === null ? undefined : entry.expiresAt,
          );
        } else {
    ...
    case "patch":
      cache.atomic(() => {
        ...
        for (const entry of op.entries)
          cache.setAt(entry.key, entry.value, entry.expiresAt);
      });
      break;
    case "transaction":
      cache.atomic(() => {
...
export class AofLog {
  ...
  append(op: AofOp): void {
    ...
    if (this.recoveryStatus === "corrupt") {
      throw new Error(
        `AOF is corrupt: ${this.recoveryDetail ?? "repair required before writing"}`,
      );
    }
    validateAofOp(op);
    if (this.lastSeq >= Number.MAX_SAFE_INTEGER)
      throw new Error("AOF sequence exhausted");
    const record: AofRecord = { version: 1, seq: this.lastSeq + 1, op };
    const encoded = JSON.stringify(record) + "\n";
    if (Buffer.byteLength(encoded) > 128 * 1024 * 1024)
      throw new DatabaseError("AOF record exceeds 128 MiB", "LIMIT_EXCEEDED");
    try {
      ...
      this.recoveryDetail =
        "AOF append failed; reopen and repair before writing";
      throw error;
    }
      ...
      try {
        ...
        if (parsed.seq !== undefined) {
          if (previousSeq !== undefined && parsed.seq <= previousSeq) {
            throw new Error(
              `AOF sequence is not increasing (${parsed.seq} after ${previousSeq})`,
            );
          }
  ...
  async replay(cache: KVCache, snapshotSeq?: number): Promise<number> {
    ...
    for (const [i, line] of lines.entries()) {
      ...
      try {
        ...
        if (
          ...
          (parsed.seq === undefined || parsed.seq <= snapshotSeq)
        )
          continue;
    ...
    if (snapshotSeq !== undefined && snapshotSeq > this.lastSeq)
  ...
  rotateAfter(snapshotSeq: number): void {
    ...
    if (this.recoveryStatus === "corrupt") {
      throw new Error(
        `AOF is corrupt: ${this.recoveryDetail ?? "repair required before rotation"}`,
      );
    }
    checkAofSeq(snapshotSeq, "snapshotSeq");
    let raw: string;
