...
export function dialSocket(host: string, port: number, options: tls.ConnectionOptions | undefined,
  timeoutMs: number, signal: AbortSignal): Promise<net.Socket> {
  return new Promise((resolve, reject) => {
    ...
    const abort = (): void => failed(new DatabaseError('connection cancelled', 'CONNECTION_CLOSED'));
    const closed = (): void => failed(new DatabaseError('connection closed during handshake', 'CONNECTION_CLOSED'));
