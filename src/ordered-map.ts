...
export class OrderedMap<K, V> implements Iterable<[K, V]> {
  ...
  constructor(
    private readonly changed?: (key: K, value: V | undefined) => void,
  ) {}
  get size(): number {
  ...
  begin(): void {
    this.frames.push(this.undo.length);
  }
  commit(): void {
    if (this.frames.pop() === undefined)
      throw new Error("No ordered-map transaction");
  ...
  private record(fn: () => void): void {
    if (this.frames.length) this.undo.push(fn);
  }
  ...
  set(key: K, value: V): this {
    ...
    if (node) {
      ...
      this.link(added, this.last);
      this.record(() => this.unlink(added));
    }
  ...
  delete(key: K): boolean {
    ...
    this.record(() => this.link(node, prev, next));
    this.unlink(node);
