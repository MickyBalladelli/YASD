...
export function valueIdentity(value: Value): string {
  ...
  return `{${Object.keys(value)
    .sort()
    .map((key) => `${JSON.stringify(key)}:${valueIdentity(value[key])}`)
    .join(",")}}`;
}
...
export class TopK<T> {
  ...
  add(value: T): void {
    ...
    if (h.length < this.k) {
      ...
      for (;;) {
        let child = i * 2 + 1;
        if (child >= h.length) break;
        if (child + 1 < h.length && this.compare(h[child + 1], h[child]) > 0)
          child++;
...
export function matchLike(value: string, pattern: string): boolean {
  ...
  while (i < text.length) {
    if (++steps > 1_000_000)
      throw new DatabaseError("LIKE work limit exceeded", "LIMIT_EXCEEDED");
    if (j < pat.length && (pat[j] === "_" || pat[j] === text[i])) {
      i++;
      j++;
    } else if (pat[j] === "%") {
      star = j++;
