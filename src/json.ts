...
export function cloneJsonValue(value: unknown, what = 'value', maxBytes = 64 * 1024 * 1024): Value {
  const ancestors = new Set<object>();
  let nodes = 0; let bytes = 0;
  const charge = (count: number): void => {
    bytes += count;
    if (bytes > maxBytes) throw new DatabaseError(`${what} exceeds JSON byte limit (${maxBytes})`, 'LIMIT_EXCEEDED');
  };
  ...
  const clone = (current: unknown, path: string, depth: number): Value => {
    ...
    if (typeof current === 'string') { stringBytes(current); return current; }
    ...
    try {
      ...
      for (const key of keys) {
        if (!array) { stringBytes(key); charge(1); }
