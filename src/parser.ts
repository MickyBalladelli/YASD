...
class Parser {
  ...
  private nextToken(): string | null {
    ...
    while (
      ...
      /\s/.test(this.state.sql[this.state.pos])
    ) {
      this.state.pos++;
    }
    ...
    const numericStart =
      ...
      (first === "." && /\d/.test(next)) ||
      (first === "-" &&
        (/\d/.test(next) || (next === "." && /\d/.test(afterNext))));
    if (numericStart) {
    ...
    for (const op of symbolOps) {
      if (
        this.state.sql.slice(this.state.pos, this.state.pos + op.length) === op
      ) {
    ...
    for (const op of wordOps) {
      const candidate = this.state.sql.slice(
        this.state.pos,
        this.state.pos + op.length,
      );
    ...
    throw new Error(
      `Unexpected character '${first}' at position ${this.state.pos}`,
    );
  }
  ...
  private validateCreateTableSchema(
  ...
  private parseValue(): Value {
    ...
    if (NUMERIC_LITERAL.test(token)) {
      ...
      if (!Number.isFinite(num))
        throw new Error(`Numeric literal out of range '${token}'`);
      return num;
    }
  ...
  private parseInteger(name: string): number {
    ...
    if (!Number.isSafeInteger(value) || value < 0) {
      throw new Error(
        `${name} must be a non-negative safe integer, got '${token}'`,
      );
    }
    this.consume();
    return value;
      ...
      if (this.peek() === "(") {
        ...
        if (this.peek() === ",") {
          if (columnType !== "number")
            throw new Error("scale requires a numeric type");
          this.consume(",");
          const scale = this.parseInteger("type scale");
          if (scale > precision)
            throw new Error("type scale exceeds precision");
        }
        this.consume(")");
      }
  ...
  private parseSelect(): SelectStatement {
    ...
    if (this.peek()?.toLowerCase() === "order") {
      ...
      if (
        this.peek()?.toLowerCase() === "asc" ||
        this.peek()?.toLowerCase() === "desc"
      ) {
...
export function parse(sql: string): SqlStatement {
  try {
    if (
      typeof sql !== "string" ||
      Buffer.byteLength(sql, "utf8") > 4 * 1024 * 1024
    ) {
