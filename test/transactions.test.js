...
async function runTests() {
  ...
  await test("embedded: failed clearPrefix restores full cache state", async () => {
    ...
    assert.strictEqual(
      db.get("outside"),
      undefined,
      "original LRU oldest entry evicted",
    );
    ...
    assert.ok(
      db.ttl("ns:one") <= beforeTtl && db.ttl("ns:one") > 0,
      "TTL deadline restored",
    );
    db.close();
  });
  ...
  await test("server/client: EXEC commits are logged + invalidate like writes", async () => {
    ...
    await sleep(100);
    assert.ok(
      invalid.some((e) => e.event === "set" && e.key === "tx:k"),
      `invalidate, got ${JSON.stringify(invalid)}`,
    );
    assert.ok(
      invalid.some((e) => e.event === "set" && e.key === "tx:n"),
      `invalidate, got ${JSON.stringify(invalid)}`,
    );
    const aofRaw = fs.readFileSync(aof, "utf8");
    assert.ok(
      aofRaw.includes('"key":"tx:k"'),
      `AOF logs tx set, got ${aofRaw}`,
    );
    const record = JSON.parse(aofRaw.trim());
    assert.strictEqual(record.op.op, "patch");
    assert.deepStrictEqual(
      record.op.entries.find((entry) => entry.key === "tx:n"),
      { key: "tx:n", value: 1 },
    );
  ...
  await test("server: tx misuse is rejected, connection stays usable", async () => {
    ...
    const convo = await new Promise((resolve, reject) => {
      ...
      sock.on("data", (c) => {
        ...
        if (
          (text.match(/(\+OK|\+QUEUED|-ERR|\$-1|\+PONG)/g) || []).length >= 9
        ) {
          sock.destroy();
          resolve(text);
        }
      ...
      setTimeout(
        () =>
          reject(new Error("raw socket test timed out: " + chunks.join(""))),
        3000,
      );
    });
    assert.ok(/EXEC without MULTI/.test(convo), `EXEC guard, got ${convo}`);
    assert.ok(
      /DISCARD without MULTI/.test(convo),
      `DISCARD guard, got ${convo}`,
    );
    assert.ok(/cannot nest/.test(convo), `nested MULTI guard, got ${convo}`);
    assert.ok(
      /not allowed inside MULTI/.test(convo),
      `queue guard, got ${convo}`,
    );
    ...
    const abort = await new Promise((resolve, reject) => {
      ...
      sock.on("error", reject);
      setTimeout(
        () => reject(new Error("abort test timed out: " + chunks.join(""))),
        3000,
      );
    });
