...
function runTests() {
  ...
  test("Create database and table", () => {
    ...
    const result = db.query(
      "CREATE TABLE users (id int, name string, age number)",
    );
    assert(
      result.affectedRows === 0,
      "Should return 0 affected rows for CREATE TABLE",
    );
  ...
  test("Insert data into table", () => {
    ...
    const result2 = db.query(
      "INSERT INTO users VALUES (2, 'Jane', 25), (3, 'Bob', 35)",
    );
  ...
  test("Delete data from table", () => {
    ...
    assert(
      !check.rows.some((r) => r.id === 2),
      "Row with id=2 should be deleted",
    );
  });
  ...
  test("Drop table", () => {
    ...
    assert(
      result.affectedRows === 0,
      "Should return 0 affected rows for DROP TABLE",
    );
  ...
  test("Reset database", () => {
    ...
    assert(
      db.getTableNames().includes("users"),
      "Table should exist before reset",
    );

    db.reset();

    assert(
      db.getTableNames().length === 0,
      "No tables should exist after reset",
    );
  });
