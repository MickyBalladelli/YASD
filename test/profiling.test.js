...
function seed() {
  ...
  for (let i = 1; i <= 200; i++) {
    db.query(
      `INSERT INTO users VALUES (${i}, '${names[i % 5]}', ${20 + (i % 30)})`,
    );
  }
  return db;
}
...
async function runTests() {
  ...
  await test("explain: AND of indexable predicates uses index-scan", async () => {
    ...
    assert.strictEqual(
      db.query("SELECT * FROM users WHERE age = 25 AND name = 'Ann'").rows
        .length,
      7,
    );
    db.close();
  ...
  await test("explain: plans agree with execution (index results are honest)", async () => {
    ...
    for (const sql of cases) {
      ...
      const scan = db.query("SELECT * FROM users").rows.filter((r) => {
        if (sql.includes("Ann") && !sql.includes("age"))
          return r.name === "Ann";
  ...
  await test("explain: ORDER BY detail and defaults", async () => {
    ...
    const combo = db.explain(
      "SELECT * FROM users WHERE name = 'Ann' ORDER BY age DESC",
    );
  ...
  await test("explain: missing table and non-SELECT statements", async () => {
    ...
    assert.throws(() => db.query("SELECT * FROM users;;"), /trailing token/);
    assert.throws(
  ...
  await test("parser hardens quotes, booleans, and numeric literals", async () => {
    const values = YASD.parse(
      "INSERT INTO t VALUES (nUlL, TrUe, FaLsE, 'it''s')",
    ).values[0];
    assert.deepStrictEqual(values, [null, true, false, "it's"]);
    const escaped = YASD.parse(
      String.raw`INSERT INTO t VALUES ('it\'s', 'line\n\t\\')`,
    ).values[0];
    assert.deepStrictEqual(escaped, ["it's", "line\n\t\\"]);

    assert.throws(
      () => YASD.parse("INSERT INTO t VALUES ('unterminated)"),
      /Unterminated quoted string/,
    );
    assert.throws(
      () => YASD.parse(String.raw`INSERT INTO t VALUES ('bad\q')`),
      /Unsupported escape/,
    );
    assert.throws(
      () => YASD.parse("INSERT INTO t VALUES (10oops)"),
      /Malformed numeric/,
    );
    assert.throws(
      () => YASD.parse("INSERT INTO t VALUES (1.2.3)"),
      /Malformed numeric/,
    );
    assert.throws(
      () => YASD.parse("INSERT INTO t VALUES (1e+)"),
      /Malformed numeric/,
    );
    assert.throws(
      () => YASD.parse("SELECT * FROM t LIMIT 1.5"),
  ...
  await test("parser rejects unknown types and invalid schemas", async () => {
    ...
    assert.throws(
      () =>
        YASD.parse(
          "CREATE TABLE missing_primary_key (id int, PRIMARY KEY (missing))",
        ),
    ...
    assert.throws(
      () =>
        YASD.parse(
          "CREATE TABLE multiple_primary_keys (id int primary key, name string primary key)",
        ),
    ...
    assert.throws(
      () =>
        YASD.parse(
          "CREATE TABLE multiple_table_primary_keys (id int, name string, PRIMARY KEY (id), PRIMARY KEY (name))",
        ),
      /Multiple primary key declarations/,
    );

    assert.doesNotThrow(() =>
      YASD.parse("CREATE TABLE without_primary_key (id int)"),
    );
  });
  ...
  await test("executor enforces constraints on INSERT and UPDATE", async () => {
    const db = new YASD({ sweepIntervalMs: 0 });
    db.query(
      "CREATE TABLE constrained (id int primary key, name string NOT NULL, enabled boolean, score number)",
    );
    ...
    assert.throws(
      () =>
        db.query(
          "INSERT INTO constrained (id, id, name, enabled, score) VALUES (2, 2, 'B', true, 20)",
        ),
    ...
    assert.throws(
      () =>
        db.query(
          "INSERT INTO constrained VALUES (2, 'B', true, 'not-a-number')",
        ),
    ...
    assert.throws(
      () =>
        db.query(
          "INSERT INTO constrained VALUES (2, 'B', 'not-a-boolean', 20)",
        ),
    ...
    assert.throws(
      () =>
        db.query(
          "INSERT INTO constrained VALUES (2, 'B', true, 20), (1, 'C', true, 30)",
        ),
      /Duplicate primary key value/,
    );
    assert.strictEqual(
      db.query("SELECT * FROM constrained WHERE id = 2").rows.length,
      0,
    );
    ...
    assert.strictEqual(
      db.query("SELECT * FROM constrained WHERE id = 1").rows.length,
      1,
    );
    assert.strictEqual(
      db.query("SELECT * FROM constrained WHERE id = 2").rows.length,
    ...
    assert.strictEqual(
      db.query("SELECT * FROM constrained WHERE id = 3").rows.length,
      1,
    );
    db.close();
  });
  ...
  await test("NULL predicates follow SQL three-valued logic", async () => {
    ...
    assert.strictEqual(
      db.query("SELECT * FROM nullable_values WHERE value = NULL").rows.length,
      0,
    );
    assert.strictEqual(
      db.query("SELECT * FROM nullable_values WHERE value != NULL").rows.length,
    ...
    assert.strictEqual(
      db.query("SELECT * FROM nullable_values WHERE NOT value = NULL").rows
        .length,
      0,
    );
    assert.deepStrictEqual(
      db
        .query("SELECT id FROM nullable_values WHERE value IS NULL")
        .rows.map((row) => row.id),
      [1],
    );
    assert.deepStrictEqual(
      db
        .query("SELECT id FROM nullable_values WHERE value IS NOT NULL")
        .rows.map((row) => row.id),
      [2],
    );
    assert.deepStrictEqual(
      db
        .query("SELECT id FROM nullable_values WHERE value IN (NULL, 'x')")
        .rows.map((row) => row.id),
      [2],
    );
    assert.deepStrictEqual(
      db
        .query("SELECT id FROM nullable_values WHERE value = NULL OR id = 1")
        .rows.map((row) => row.id),
      [1],
    );

    const update = db.query(
      "UPDATE nullable_values SET id = 9 WHERE value != NULL",
    );
    assert.strictEqual(update.affectedRows, 0);
    assert.deepStrictEqual(
      db.query("SELECT id FROM nullable_values").rows.map((row) => row.id),
      [1, 2],
    );
    db.close();
  });
  ...
  await test("referenced columns must exist", async () => {
    ...
    assertColumnNotFound(() =>
      db.query("SELECT * FROM users ORDER BY missing"),
    );
    assertColumnNotFound(() =>
      db.query("SELECT * FROM users WHERE missing = 1"),
    );
    assertColumnNotFound(() => db.explain("SELECT missing FROM users"));
    assertColumnNotFound(() =>
      db.query("UPDATE users SET age = 1 WHERE missing = 1"),
    );
  ...
  await test("ORDER BY keeps NULL last and mixed values deterministic", async () => {
    ...
    db.query(
      "INSERT INTO order_values VALUES (1, NULL), (2, '10'), (3, 2), (4, '2'), (5, false), (6, true), (7, '10')",
    );

    assert.deepStrictEqual(
      db
        .query("SELECT id FROM order_values ORDER BY value")
        .rows.map((row) => row.id),
      [3, 2, 7, 4, 5, 6, 1],
    );
    assert.deepStrictEqual(
      db
        .query("SELECT id FROM order_values ORDER BY value DESC")
        .rows.map((row) => row.id),
      [6, 5, 4, 2, 7, 3, 1],
    );
    db.close();
  ...
  await test("index configuration plans writes around available indexes", async () => {
    ...
    db.query(
      "CREATE TABLE indexed_writes (id int primary key, bucket number, label string)",
    );
    db.query(
      "INSERT INTO indexed_writes VALUES (1, 10, 'a'), (2, 20, 'b'), (3, 30, 'c')",
    );

    assert.strictEqual(
      db.explain("SELECT * FROM indexed_writes WHERE id = 2").strategy,
      "index-scan",
    );
    assert.strictEqual(
      db.explain("SELECT * FROM indexed_writes WHERE bucket = 20").strategy,
      "full-scan",
    );

    db.query("UPDATE indexed_writes SET label = 'updated' WHERE id = 2");
    assert.strictEqual(
      db.query("SELECT label FROM indexed_writes WHERE id = 2").rows[0].label,
      "updated",
    );
    db.query("DELETE FROM indexed_writes WHERE id = 1");
    assert.strictEqual(
      db.query("SELECT * FROM indexed_writes WHERE id = 1").rows.length,
      0,
    );
    db.close();
    ...
    assert.strictEqual(
      noIndexes.explain("SELECT * FROM unindexed WHERE id = 1").strategy,
      "full-scan",
    );
    noIndexes.close();

    assert.throws(
      () => new YASD({ indexColumns: ["id", "id"] }),
      /duplicate index column/,
    );
    assert.throws(
      () => new YASD({ indexColumns: ["bad-name"] }),
  ...
  await test("profile: SELECT reports plan + timing + row counts", async () => {
    const db = seed();
    const sql =
      "SELECT * FROM users WHERE name = 'Ann' ORDER BY age DESC LIMIT 5";
    ...
    assert.strictEqual(
      p.rowsReturned,
      5,
      "LIMIT applied (40 Ann rows, 5 returned)",
    );
  ...
  await test("profile: full-scan SELECT and write statements", async () => {
    ...
    assert.strictEqual(
      db.query("SELECT * FROM users WHERE name = 'Zed'").rows.length,
      0,
    );
    db.close();
  });
  ...
  await test("profile: runs feed the slow-query log when over threshold", async () => {
    ...
    assert.ok(
      log[0].sql.includes("Ann"),
      `profiled sql logged, got ${log[0].sql}`,
    );
    db.close();
  });
