...
async function wire(t, port) {
  ...
  return {
    ...
    async read(n) {
      ...
      assert.ok(
        replies.length >= n,
        `expected ${n} replies; got ${JSON.stringify(replies)}`,
      );
...
test("SQL: assignment commas, parentheses, BETWEEN and strict statement grammar", () => {
  ...
  try {
    ...
    assert.deepEqual(db.query("SELECT a, b FROM t WHERE id = 5").rows, [
      { a: "new", b: 3 },
    ]);
    assert.deepEqual(
      db.query("SELECT id FROM t WHERE NOT (id BETWEEN 10 AND NULL)").rows,
      [{ id: 5 }],
    );
    for (const sql of [
      "SELECT * FROM t;;",
      ...
      "CREATE TABLE bad (id varchar(oops))",
      "CREATE TABLE bad (id numeric(2,3))",
      ...
      assert.throws(
        () => db.query(sql),
        (error) =>
          error instanceof DatabaseError && error.code === "PARSE_ERROR",
        sql,
      );
    }
  } finally {
...
test("SQL: owned schemas and special property names", () => {
  ...
  try {
    db.query(
      "CREATE TABLE special (id int primary key, __proto__ string, constructor string)",
    );
    db.query("INSERT INTO special VALUES (1, 'safe', 'ctor')");
    const schema = db.getTableSchema("special");
    schema.primaryKey = undefined;
    schema.columns[0].type = "string";
    schema.columns.length = 0;
    assert.throws(
      () => db.query("INSERT INTO special VALUES (1, 'bad', 'bad')"),
      (error) => error.code === "PRIMARY_KEY_CONSTRAINT",
    );
    assert.deepEqual(
      db.query("SELECT * FROM special").rows[0],
      JSON.parse('{"id":1,"__proto__":"safe","constructor":"ctor"}'),
    );
  } finally {
    db.close();
  }
...
test("JSON contract: embedded and wire reject normalization, getters and hooks", async (t) => {
  ...
  for (const value of [
    ...
    await assert.rejects(
      client.set("bad", value),
      (error) => error.code === "INVALID_VALUE",
    );
  }
  assert.equal(calls, 0);
  const value = JSON.parse('{"__proto__":{"safe":true},"nested":[1]}');
  cache.set("owned", value);
  await client.set("owned", value);
  value.nested.push(2);
...
test("CAS retains persistent namespace keys; WATCH catches expiry and tombstone ABA", () => {
  ...
  try {
    db.set("feeds:x", 1);
    db.persist("feeds:x");
    assert.equal(db.cas("feeds:x", 1, 2), true);
    assert.equal(db.ttl("feeds:x"), -1);
    db.set("watched", 1, 10);
    const expired = db.multi();
    expired.watch("watched");
    expired.set("out", 1);
    now += 20;
    assert.equal(expired.exec(), null);
    db.set("watched", 1);
    const aba = db.multi();
    aba.watch("watched");
    db.del("watched");
...
test("snapshot replacement rejects bad values, deadlines and duplicate keys atomically", async (t) => {
  ...
  const cache = new KVCache({ sweepIntervalMs: 0, maxValueBytes: 20 });
  t.after(() => cache.close());
  cache.set("preserved", 1, 60_000);
  const before = cache.dump();
  const stats = cache.stats();
  const version = cache.getVersion("preserved");
  for (const entries of [
    [{ key: "bad", value: "x".repeat(30) }],
    [{ key: "bad", value: 1, expiresAt: "never" }],
    [
      { key: "dup", value: 1 },
      { key: "dup", value: 2 },
    ],
...
test("AOF: final state retains PERSIST/extension and never resurrects expired INCR", async (t) => {
  ...
  try {
    server.cache.set("expired", 10, 100);
    server.cache.incr("expired");
    server.cache.set("persistent", 7, 100);
    server.cache.persist("persistent");
    server.cache.set("extended", 8, 100);
    server.cache.expire("extended", 1000);
    now += 200;
    const cache = new KVCache({ sweepIntervalMs: 0 });
    try {
      const log = new AofLog(aofPath);
      await log.replay(cache);
      assert.equal(log.recoveryState, "clean");
      assert.equal(cache.get("expired"), undefined);
      assert.equal(cache.get("persistent"), 7);
      assert.equal(cache.ttl("persistent"), -1);
      assert.equal(cache.get("extended"), 8);
      assert.equal(cache.ttl("extended"), 800);
    } finally {
      cache.close();
    }
  } finally {
...
test("AOF: eviction tombstones preserve membership despite unlogged reads", async (t) => {
  ...
  const server = new YasdServer({
    aofPath,
    cache: { maxEntries: 2, sweepIntervalMs: 0 },
  });
  t.after(() => server.close());
  server.cache.set("a", 1);
  server.cache.set("b", 2);
  server.cache.get("a");
  server.cache.set("c", 3);
  const cache = new KVCache({ maxEntries: 2, sweepIntervalMs: 0 });
  t.after(() => cache.close());
...
test("alternate SAVE is an export; missing AOF still advances snapshot sequence", async (t) => {
  ...
  const snapshotPath = file("main.json");
  const aofPath = file("main.aof");
  let server = new YasdServer({
    snapshotPath,
    aofPath,
    port: 0,
    saveOnShutdown: false,
  ...
  await server.save(file("export.json"));
  await server.close();
  server = new YasdServer({
    snapshotPath,
    aofPath,
    port: 0,
    saveOnShutdown: false,
  ...
  await server.start();
  assert.equal(server.cache.get("new"), 2);
  await server.save();
  await server.close();
  ...
  await server.start();
  server.cache.set("after-missing", 3);
  await server.close();
  server = new YasdServer({
    snapshotPath,
    aofPath,
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      op: {
        ...
        entries: [
          { key: "first", value: 1 },
          { key: "invalid", value: "x".repeat(50) },
        ],
      },
    }) + "\n",
  );
  const cache = new KVCache({ maxValueBytes: 20, sweepIntervalMs: 0 });
  t.after(() => cache.close());
  cache.set("before", 7);
  const log = new AofLog(file);
  assert.equal(await log.replay(cache), 0);
  assert.equal(log.recoveryState, "corrupt");
  assert.equal(cache.get("first"), undefined);
  assert.equal(cache.get("before"), 7);
});
...
test("AOF truncation scanner accepts only valid incomplete JSON prefixes", () => {
  const record = JSON.stringify({
    version: 1,
    seq: 1,
    op: { op: "set", key: 'quoted"', value: [null, true, -1.5e23] },
  });
  for (let i = 1; i < record.length; i++)
    assert.equal(isIncompleteJson(record.slice(0, i)), true, `cut ${i}`);
...
test("AOF partial append fails closed and memory rolls back", async (t) => {
  ...
  try {
    assert.throws(() => server.cache.set("new", 2), /AOF write failed/);
  } finally {
  ...
  assert.equal(log.recoveryState, "torn-tail");
...
test("wire FIFO: SAVE, write, LOAD, GET, PING and QUIT remain ordered", async (t) => {
  const { server } = await running(t, {
    snapshotPath: fixture(t)("fifo.json"),
  });
...
test("RESP protocol choice survives every alphabetic payload split", async (t) => {
  ...
  for (let i = 1; i < frame.length; i++) {
    raw.socket.write(frame.subarray(0, i));
    await pause(1);
    raw.socket.write(frame.subarray(i));
...
test("HTTP rejects a complete oversized header", async (t) => {
  ...
  await end;
  assert.match(text, /^HTTP\/1.1 431/);
});
...
test("client transactions: empty exec, fresh reads, concurrent first writes and parent close", async (t) => {
  ...
  assert.equal(await tx.get("read"), 1);
  assert.equal(await tx.ttl("read"), -1);
  assert.deepEqual(await tx.mget(["read"]), [1]);
  await Promise.all([tx.set("a", 1), tx.set("b", 2)]);
  assert.deepEqual(await tx.exec(), ["OK", "OK"]);
  const abandoned = client.multi();
  await abandoned.watch("read");
  await client.close();
...
test("transaction queue limits abort without applying the accepted prefix", async (t) => {
  ...
  assert.equal(replies[1].value, "QUEUED");
  assert.equal(replies[2].kind, "error");
  assert.equal(replies[3].kind, "error");
  assert.equal(replies[4].value, "PONG");
...
test("large finite counter results and fractional TTLs remain decodable", async (t) => {
  const { client } = await running(t);
  await client.set("large", 1e20);
  assert.equal(await client.incr("large", 1e20), 2e20);
  await client.set("fraction", 1, 10000.5);
  assert.ok(Number.isSafeInteger(await client.ttl("fraction")));
  assert.throws(
    () => require("../dist/protocol").encodeInt(Number.MAX_SAFE_INTEGER + 1),
    /safe integers/,
  );
});
...
test("configuration: negative flags, false env, precedence and secret redaction", () => {
  ...
  assert.equal(options.loadOnStart, false);
  assert.equal(options.saveOnShutdown, false);
  assert.equal(options.port, 0);
  assert.equal(options.maxWatchedKeys, 7);
  assert.equal(
    resolveCliOptions([], { YASD_LOAD_ON_START: "false" }).loadOnStart,
    false,
  );
  assert.throws(() => resolveCliOptions(["--unknown"], {}), /unknown/);
  assert.throws(() => new YasdServer({ autoSaveMs: 2 ** 32 }), /autoSaveMs/);
  assert.throws(
    () => new YasdServer({ loadOnStart: "false" }),
    /true or false/,
  );
  assert.throws(
    () => new YasdServer({ snapshotPath: "same", aofPath: "./same" }),
    /different/,
  );
  assert.throws(
    () => new YasdClient({ url: "yasd://:private%ZZ@localhost" }),
...
test("lifecycle: joined start/close and truthful shutdown failure", async (t) => {
  ...
  assert.equal(server.start(), start);
  await start;
  ...
  const invalid = fixture(t)("directory");
  fs.mkdirSync(invalid);
  const broken = new YasdServer({
    snapshotPath: invalid,
    saveOnShutdown: true,
    loadOnStart: false,
    port: 0,
  });
...
test("TCP/TLS handshake deadline and close cancel pending dials", async (t) => {
  ...
  await assert.rejects(tx.connect(), (error) => error.code === "TIMEOUT");
  await tx.close();
  const cancelled = new YasdTransaction({
    host: "127.0.0.1",
    port: blackhole.address().port,
    tlsOptions: { rejectUnauthorized: false },
    connectTimeoutMs: 3000,
  });
  const attempt = cancelled.connect();
  const assertion = assert.rejects(attempt);
  await pause(5);
