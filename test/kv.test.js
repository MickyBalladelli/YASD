...
async function runTests() {
  ...
  await test("KV owns mutable values at the cache boundary", () => {
    ...
    assert.deepStrictEqual(db.get("mutable"), {
      nested: { items: ["before"] },
    });
  ...
  await test("CAS compares nested JSON independent of key order", () => {
    ...
    for (let i = 0; i < 1000; i++)
      large[`key-${i}`] = { value: i, nested: [i, i % 2 === 0] };
  ...
  await test("namespace TTL defaults apply", () => {
    ...
    assert.ok(
      popular > 55000 && popular <= 60000,
      `popular ttl ~60s, got ${popular}`,
    );
    db.set("plain", 1);
  ...
  await test("indexed WHERE =, IN, AND", () => {
    const db = new YASD({ sweepIntervalMs: 0 });
    db.query(
      "CREATE TABLE users (id int primary key, name string, age number)",
    );
    ...
    const scan = db
      ...
      .rows.filter((r) => r.age === 25 && r.name === "Bob");
    assert.strictEqual(
      andRes.rows.length,
      scan.length,
      "AND matches full scan",
    );
  ...
  await test("WHERE preserves literal and column-reference identity", () => {
    ...
    db.query(
      "INSERT INTO people VALUES ('age', '30'), ('bob', 'age'), ('age', 'age')",
    );
    const literal = db.query("SELECT * FROM people WHERE name = 'age'");
    assert.deepStrictEqual(
      literal.rows.map((row) => row.name),
      ["age", "age"],
    );
  ...
  await test("delete then index reads stay correct", () => {
    ...
    assert.ok(
      rest.rows.every((r) => r.v !== "v1"),
      "no deleted values remain",
    );
    assert.strictEqual(
      db.query("SELECT * FROM t WHERE v = 'v0'").rows.length,
      rest.rows.filter((r) => r.v === "v0").length,
      "index read matches scan after delete",
    );
    db.close();
  });
  ...
  await test("parser handles >=, !=, BETWEEN, LIKE", () => {
    ...
    db.query(
      "INSERT INTO u VALUES (1, 'John', 30), (2, 'Jane', 25), (3, 'Bob', 35)",
    );
    assert.strictEqual(
      db.query("SELECT * FROM u WHERE age >= 30").rows.length,
      2,
    );
    assert.strictEqual(
      db.query("SELECT * FROM u WHERE name != 'John'").rows.length,
      2,
    );
    assert.strictEqual(
      db.query("SELECT * FROM u WHERE age BETWEEN 24 AND 31").rows.length,
      2,
    );
    assert.strictEqual(
      db.query("SELECT * FROM u WHERE name LIKE 'J%'").rows.length,
      2,
    );
    db.close();
  });
