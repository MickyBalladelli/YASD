...
test("runtime LOAD survives restart as an atomic AOF replacement", async () => {
  ...
  const options = {
    ...
    aofPath: path.join(dir, "cache.aof"),
    saveOnShutdown: false,
  };
  let server = new YasdServer(options);
  try {
    await server.start();
    server.cache.set("saved", 1);
    await server.save();
    server.cache.set("later", 2);
    await server.load();
    assert.equal(server.cache.get("later"), undefined);
    await server.close();
    server = new YasdServer(options);
    await server.start();
    assert.equal(server.cache.get("saved"), 1);
    assert.equal(server.cache.get("later"), undefined);
  } finally {
    await server.close();
...
test("failed EXEC writes neither AOF nor invalidation, and restores all values", async () => {
  ...
  const server = new YasdServer({ port: 0, aofPath });
  let client;
  try {
    await server.start();
    ...
    server.cache.set("not-number", "text");
    const before = fs.readFileSync(aofPath, "utf8");
    const events = [];
    const unsubscribe = server.pubsub.subscribe(
      "__yasd__:invalidate",
      (_, event) => events.push(event),
    );
    const tx = client.multi();
    await tx.set("first", 1);
    await tx.incr("not-number");
    await assert.rejects(tx.exec());
    assert.equal(server.cache.get("first"), undefined);
    assert.equal(server.cache.get("not-number"), "text");
    assert.equal(fs.readFileSync(aofPath, "utf8"), before);
...
test("persistence rejects invalid UTF-8 and oversized files without replacing state", async () => {
  ...
  try {
    ...
    fs.writeFileSync(
      bad,
      Buffer.concat([
        Buffer.from('{"op":"set","key":"x","value":"'),
        Buffer.from([0xff]),
        Buffer.from('"}\n'),
      ]),
    );
    ...
    fs.ftruncateSync(fd, 257 * 1024 * 1024);
    fs.closeSync(fd);
...
test("largest cache values remain readable; oversized MGET/EXEC fail without committing", async () => {
  ...
  try {
    ...
    await client.set("big", value);
    assert.equal((await client.get("big")).length, value.length);
...
test("remote SAVE/LOAD cannot select arbitrary filesystem paths", async () => {
  ...
  try {
    ...
    await assert.rejects(
      client.save("/not-allowed/snapshot.json"),
      (error) => error.code === "AUTH_ERROR",
    );
    await assert.rejects(
      client.load("/not-allowed/snapshot.json"),
      (error) => error.code === "AUTH_ERROR",
    );
    assert.equal(await client.ping(), "PONG");
  } finally {
...
test("autosave coalesces rather than growing an unbounded persistence queue", async () => {
  ...
  try {
    await server.start();
    server.cache.set("key", 1);
    for (let i = 0; i < 100 && !fs.existsSync(server.snapshotPath); i++)
      await delay(3);
    assert.ok(fs.existsSync(server.snapshotPath));
    assert.ok(server.persistenceDepth <= 1);
  } finally {
    await server.close();
    fs.rmSync(dir, { recursive: true, force: true });
  }
