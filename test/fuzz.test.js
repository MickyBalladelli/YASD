...
function parserProperties() {
  ...
  const numbers = ['0', '-0', '1', '-42', '3.14', '.5', '10.', '1e3', '-2.5e-2']
  ...
  for (let i = 0; i < 400; i++) {
    ...
    assert.deepStrictEqual(statement.values[0], [Number(number), text, boolean])
  }
  ...
  for (let i = 0; i < 100; i++) {
    assertError(() => parse(base + suffixes[int(random, 0, suffixes.length - 1)]), /trailing token/)
  }

  for (let i = 0; i < 250; i++) {
    const garbage = randomText(random, 120) + String.fromCharCode(int(random, 0, 31))
    try {
      parse(garbage)
    } catch (error) {
      assert.ok(error instanceof Error)
...
function respProperties() {
  ...
  for (let i = 0; i < 300; i++) {
    const args = [
      'SET',
      `fuzz:${i}`,
      JSON.stringify({ text: randomText(random), value: int(random, -1000, 1000) }),
      'PX',
  ...
  assertError(
    () => new RespDecoder({ maxBufferedBytes: 1024 }).push(Buffer.alloc(2048, 0x2a)),
