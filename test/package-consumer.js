...
function run(command, args, cwd = dir) { return execFileSync(command, args, { cwd, encoding: 'utf8', timeout: 120_000 }); }
try {
  ...
  fs.writeFileSync(path.join(dir, 'package.json'), '{"name":"yasd-consumer","private":true}');
  run(npm, ['install', '--ignore-scripts', '--no-audit', '--no-fund', path.join(dir, files[0].filename)]);
  fs.writeFileSync(path.join(dir, 'consumer.cjs'), `
    const assert = require('node:assert/strict');
  ...
  run(process.execPath, [path.join(root, 'node_modules/typescript/bin/tsc'), '--noEmit', '--strict', '--target', 'ES2020', '--module', 'node16', '--moduleResolution', 'node16', '--typeRoots', path.join(root, 'node_modules/@types'), 'consumer.ts']);
  assert.match(run(process.execPath, ['node_modules/yasd/dist/cli.js', '--help']), /tls-min-version/);
  const stage = path.join(dir, 'dependency-stage'); fs.mkdirSync(stage);
  ...
  console.log('Installed package: CommonJS, ESM, declarations, version, CLI and package-only npm ci passed.');
