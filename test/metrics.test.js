...
async function runTests() {
  ...
  await test("hits/misses counted on get/mget/ttl", async () => {
    ...
    assert.ok(
      db.ttl("a") === -1 || db.ttl("a") >= 0,
      "live ttl read (no counter change)",
    );
  ...
  await test("SQL slow-query log: off by default, threshold + cap + clear", async () => {
    ...
    assert.ok(
      log[0].sql.includes("SELECT"),
      `sql text kept, got ${JSON.stringify(log[0])}`,
    );
  ...
  await test("server slow-command log records with tiny threshold", async () => {
    ...
    assert.ok(
      names.includes("SET"),
      `SET logged, got ${JSON.stringify(names)}`,
    );
    assert.ok(
      names.includes("GET"),
      `GET logged, got ${JSON.stringify(names)}`,
    );
    for (const e of info.slowLog) {
      assert.ok(e.durationMs >= 0, "duration measured");
      assert.ok(e.at > 0, "timestamp set");
      assert.ok(
        Number.isInteger(e.argc) && e.argc >= 0,
        `argc, got ${JSON.stringify(e)}`,
      );
    }
  ...
  await test("serverOptionsFromEnv parses YASD_SLOW_COMMAND_MS", async () => {
    ...
    assert.throws(
      () => serverOptionsFromEnv({ YASD_SLOW_COMMAND_MS: "nope" }),
      />= 0/,
    );
    assert.throws(
      () => serverOptionsFromEnv({ YASD_SLOW_COMMAND_MS: "-1" }),
      />= 0/,
    );
  });
  ...
  await test("operational limits are validated and persistence status is safe", async () => {
    ...
    assert.throws(
      () => serverOptionsFromEnv({ YASD_MAX_COMMAND_MS: "3000000000" }),
      /<=/,
    );
