...
async function runTests() {
  ...
  await test("incr/decr from missing, floats, TTL preserved", async () => {
    ...
    assert.ok(
      db.ttl("t") > 1000 && db.ttl("t") <= 5000,
      "TTL preserved across incr",
    );
    db.set("s", "nope", 5000);
  ...
  await test("mget/mset round trip with TTLs", async () => {
    ...
    assert.deepStrictEqual(db.mget(["a", "b", "missing"]), [
      1,
      { posts: [1, 2] },
      undefined,
    ]);
  ...
  await test("snapshot save/load round trip (expired skipped)", async () => {
    ...
    assert.strictEqual(
      await loadSnapshot(db2, path.join(dir, "nope.json")),
      0,
      "missingOk",
    );
    db.close();
  ...
  await test("AOF append + replay", async () => {
    ...
    fs.appendFileSync(
      aofPath,
      '{"version":1,"seq":8,"op":{"op":"set","key":"partial"',
    );
  ...
  await test("AOF replay stops at middle corruption and exposes recovery state", async () => {
    ...
    assert.throws(
      () => log.append({ op: "set", key: "blocked", value: true }),
      /AOF is corrupt/,
    );
    ...
    assert.strictEqual(
      server.persistenceStatus().errors[0].code,
      "AOF_CORRUPT",
    );
  ...
  await test("AOF replay preserves absolute TTL deadlines", async () => {
    ...
    assert.ok(
      remaining > 0 && remaining <= deadline - afterReplay + 5,
      "TTL was not restarted from replay time",
    );
    await sleep(100);
    assert.strictEqual(
      db.get("ttl"),
      undefined,
      "original deadline still wins after replay",
    );
    db.close();
  });
  ...
  await test("snapshot/AOF recovery skips superseded records after crash", async () => {
    ...
    assert.strictEqual(
      recovered.get("counter"),
      4,
      "old AOF record was not replayed twice",
    );
    ...
    assert.strictEqual(
      rotated.get("counter"),
      4,
      "rotated AOF kept the post-snapshot tail",
    );
    db.close();
    recovered.close();
    rotated.close();
  ...
  await test("protocol encode/decode incl. partial chunks", async () => {
    ...
    assert.deepStrictEqual(requestArgv(out[0]), [
      "SET",
      "k",
      '{"a":1}',
      "PX",
    ...
    assert.deepStrictEqual(replies[3], {
      kind: "array",
      items: [
        { kind: "bulk", value: "x" },
  ...
  await test("protocol decoder enforces frame, bulk, args, depth, and buffer limits", async () => {
    ...
    assert.throws(
      () => new RespDecoder({ maxFrameBytes: 4 }).push("+OK\r\n"),
      /frame exceeds/,
    );
    assert.throws(
      () => new RespDecoder({ maxBulkBytes: 3 }).push("$4\r\n1234\r\n"),
      /bulk length exceeds/,
    );
    assert.throws(
      () =>
        new RespDecoder({ maxArguments: 1 }).push(
          "*2\r\n$1\r\na\r\n$1\r\nb\r\n",
        ),
      /array length exceeds/,
    );
    assert.throws(
      () => new RespDecoder({ maxDepth: 1 }).push("*1\r\n*1\r\n$1\r\na\r\n"),
      /nesting exceeds/,
    );
    assert.throws(
      () => new RespDecoder({ maxBufferedBytes: 4 }).push("*9999"),
  ...
  await test("CACHE_URL parsing + fromEnv", async () => {
    assert.deepStrictEqual(
      parseCacheUrl("yasd://cache.internal:7379?poolSize=8"),
      {
        host: "cache.internal",
        port: 7379,
        poolSize: 8,
    ...
    assert.deepStrictEqual(parseCacheUrl("yasd://127.0.0.1"), {
      host: "127.0.0.1",
      port: 7379,
    });
    assert.deepStrictEqual(
      parseCacheUrl("yasd://:p%40ss@cache.internal?password=q%2Br"),
      {
        host: "cache.internal",
        port: 7379,
        password: "q+r",
    ...
    assert.throws(
      () => parseCacheUrl("yasd://user%ZZ:pass@cache.internal"),
      /encoding/,
    );
    assert.throws(() => parseCacheUrl("yasd://cache.internal:65536"), /port/);
    assert.throws(
      () => parseCacheUrl("yasd://cache.internal?poolSize=10oops"),
      /poolSize/,
    );
    assert.throws(
      () => parseCacheUrl("yasd://cache.internal?poolSize=1025"),
      /poolSize/,
    );
    ...
    assert.throws(
      () => new YasdClient({ requestTimeoutMs: NaN }),
      /requestTimeoutMs/,
    );
    assert.throws(
      () => new YasdClient({ requestTimeoutMs: 2_147_483_648 }),
      /requestTimeoutMs/,
    );
    assert.throws(
      () => new YasdClient({ url: "yasds://cache.internal", tls: false }),
      /TLS/,
    );
    assert.throws(
      () => new YasdClient({ url: "yasd://cache.internal", tls: true }),
      /TLS/,
    );
    assert.throws(
      () => new YasdClient({ url: "yasd://cache.internal", tls: {} }),
      /TLS/,
    );
    assert.throws(() => new YasdClient({ tls: null }), /TLS/);
    const c = YasdClient.fromEnv({
      CACHE_URL: "yasd://127.0.0.1:7379?poolSize=2",
    });
  ...
  await test("server/client: ping, kv, ttl, counters, batch", async () => {
    ...
    assert.deepStrictEqual(await client.mget(["m1", "m2", "nope"]), [
      1,
      [2],
      undefined,
    ]);
  ...
  await test("server/client: pipelined batch under pool", async () => {
    ...
    await client.connect();
    await Promise.all(
      Array.from({ length: 30 }, (_, i) => client.set(`p:${i}`, i)),
    );
    const got = await Promise.all(
      Array.from({ length: 30 }, (_, i) => client.get(`p:${i}`)),
    );
    assert.deepStrictEqual(
      got,
  ...
  await test("server/client: dead pooled connections are removed and replenished", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      assert.strictEqual(
        client.pool.length,
        2,
        "pool stays bounded after replacement",
      );
    }
  ...
  await test("server: slow consumers are disconnected at the output limit", async () => {
    ...
    try {
      ...
      await waitFor(
        () => server.sockets.size === 1,
        "server accepted slow socket",
      );
      ...
      try {
        raw.write("*2\r\n$9\r\nSUBSCRIBE\r\n$4\r\nslow\r\n");
        await waitFor(
          () => server.pubsub.channelNames().includes("slow"),
        ...
        await Promise.race([
          closed,
          sleep(1000).then(() => {
            throw new Error("slow consumer stayed connected");
          }),
        ]);
  ...
  await test("server/client: pub/sub + invalidation event", async () => {
    ...
    await sleep(100);
    assert.deepStrictEqual(received[received.length - 1], [
      "presence",
      "after-reconnect",
    ]);
    assert.deepStrictEqual(receivedSecond[receivedSecond.length - 1], [
      "presence",
      "after-reconnect",
    ]);
    ...
    const invalid = [];
    const unsub2 = await sub.subscribe("__yasd__:invalidate", (ch, msg) =>
      invalid.push(JSON.parse(msg)),
    );
    await pub.set("feeds:x", { p: [] });
    await sleep(100);
    assert.ok(
      invalid.some((e) => e.event === "set" && e.key === "feeds:x"),
      `invalidate, got ${JSON.stringify(invalid)}`,
    );
  ...
  await test("server: PERSIST, expiry, and LOAD publish invalidations", async () => {
    ...
    const invalid = [];
    const unsub = await sub.subscribe("__yasd__:invalidate", (ch, msg) =>
      invalid.push(JSON.parse(msg)),
    );
    ...
    assert.ok(
      invalid.some((e) => e.event === "persist" && e.key === "persist:key"),
    );
    assert.ok(
      invalid.some((e) => e.event === "expire" && e.key === "expire:key"),
    );
  ...
  await test("server: CAS PX 0 stays absent after AOF replay", async () => {
    ...
    assert.strictEqual(
      await client.cas("cas:zero", { old: true }, { new: true }, 0),
      true,
    );
    ...
    assert.ok(
      records.some(
        (record) =>
          record.op?.op === "patch" && record.op.deleted.includes("cas:zero"),
      ),
    );
    const recovered = new YasdServer({
      host: "127.0.0.1",
      port: 0,
      aofPath: aof,
    });
  ...
  await test("server: /healthz + SAVE/LOAD + graceful close", async () => {
    ...
    const rawHttp = (request, targetPort = port) =>
      new Promise((resolve, reject) => {
        const chunks = [];
        let timer;
        const sock = require("net").createConnection(
    ...
    assert.match(
      await rawHttp("GET /livez HTTP/1.1\r\nHost: localhost\r\n\r\n"),
      /^HTTP\/1\.1 200 OK/,
    );
    assert.match(
      await rawHttp("GET /readyz HTTP/1.1\r\nHost: localhost\r\n\r\n"),
      /^HTTP\/1\.1 200 OK/,
    );
    assert.match(
      await rawHttp("POST /healthz HTTP/1.1\r\nHost: localhost\r\n\r\n"),
      /^HTTP\/1\.1 405 Method Not Allowed/,
    );
    assert.match(
      await rawHttp("GET /healthz?bad=%ZZ HTTP/1.1\r\nHost: localhost\r\n\r\n"),
      /^HTTP\/1\.1 400 Bad Request/,
    );
    assert.match(
      await rawHttp("GET /healthz HTTP/1.0\r\nHost: localhost\r\n\r\n"),
    ...
    assert.deepStrictEqual(
      await client.healthcheck().then((h) => h.status),
      "ok",
    );
  ...
  await test("server rejects bad input without dropping the connection", async () => {
    ...
    const raw = await new Promise((resolve, reject) => {
      ...
      setTimeout(
        () => reject(new Error("raw socket test timed out: " + lines.join(""))),
        3000,
      );
    });
    ...
    assert.strictEqual(
      await client.ping(),
      "PONG",
      "client connection still usable",
    );
    await client.close();
    await server.close();
  });
